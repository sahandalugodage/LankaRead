    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 p-6 rounded-2xl border border-slate-800 backdrop-blur-md shadow-xl">
        <div>
          <div className="flex items-center space-x-2 text-teal-400 text-xs font-bold uppercase tracking-wider mb-1">
            ...
            <span>LankaRead Members Directory</span>
          </div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Library Members</h1>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            <MdSync className={`w-5 h-5 ${isReadersLoading ? "animate-spin text-teal-400" : ""}`} />
          </button>
          <button
      ...
      {isReadersLoading && (
        <div className="p-12 text-center text-slate-400 font-medium animate-pulse">
          Loading LankaRead member profiles...
        </div>
      )}
      ...
      <Dialog isOpen={isDeleteDialogOpen} onCancel={cancelDialog} onConfirm={confirmDelete} title="Revoke Member Card">
        <p className="text-slate-300 text-sm leading-relaxed">
          Are you sure you want to remove <strong className="text-white">{selectedReader?.name}</strong> from LankaRead membership?
        </p>
      </Dialog>
    </div>
  );
