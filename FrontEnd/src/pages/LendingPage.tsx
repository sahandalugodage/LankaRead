    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 p-6 rounded-2xl border border-slate-800 backdrop-blur-md shadow-xl">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Book Circulation & Loans</h1>
          <p className="text-xs text-slate-400 mt-1">Track active loans, due dates, returns, and overdue fines</p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            title="Refresh Loans"
          >
            <MdSync className={`w-5 h-5 ${isLoading ? "animate-spin text-teal-400" : ""}`} />
          </button>
          <button
      ...
      {isLoading && (
        <div className="p-12 text-center text-slate-400 font-medium animate-pulse">
          Loading LankaRead loan registers...
        </div>
      )}
