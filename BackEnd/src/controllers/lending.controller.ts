...
export const sendOverdueNotification = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const text = `Dear ${lending.readerName},\n\n` +
            ...
            `issued to your LankaRead membership account is overdue.\n` +
            `Due Date: ${lending.dueDate ? new Date(lending.dueDate).toLocaleDateString() : "N/A"}\n\n` +
            `Please return the book to your nearest LankaRead branch at your earliest convenience to avoid fine accumulation.\n\n` +
            `Thank you,\nLankaRead Library Network`;
